...
final class SessionManager: ObservableObject {
    ...
    /// Re-reads the shared-secret's presence from Keychain and updates
    /// `isProvisioned` immediately. Call this right after saving (or
    /// clearing) the HMAC secret in Settings/CredentialsView — without
    /// this, isProvisioned only ever reflected keychain state as of app
    /// launch, so a freshly-saved secret wouldn't take effect until the
    /// user force-quit and relaunched the app.
    func refreshProvisioning() {
        isProvisioned = keychainService.exists(.sharedSecret)
    }
}
