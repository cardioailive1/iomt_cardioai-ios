...
// NOTE: No explicit CodingKeys — APIClient's decoder uses
// `.convertFromSnakeCase`, which maps `access_token` → `accessToken` etc.
// Adding snake_case CodingKeys here BREAKS decoding: the strategy converts
// the JSON key first, then it no longer matches the snake_case rawValue.
