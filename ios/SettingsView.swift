            ...
            Form {
                ...
                if let user = authService.currentUser {
                    Section("Account") {
                        ...
                        // visible to everyone, patients included (unlike
                        // the technical Backend Connection/Security
                        // sections above, which are gated to clinical
                        // staff only, this applies to every account type).
                        NavigationLink("Subscription & Account") {
                ...
                // ── Connection (hardware/BLE-gateway bridge) ─────────────────
                //
                // This entire section describes the real-time WebSocket
                // hardware bridge — a hospital-IT-provisioned feature that
                // ordinary patients never touch (see RootView.swift and
                // DashboardView's connection chip, which already hide this
                // same concept elsewhere). Showing raw connection status,
                // backend hostnames, and environment names to a patient is
                // pure technical noise; clinical/admin users may still need
                // it for support purposes, so it stays visible for them.
                if !isPatient {
